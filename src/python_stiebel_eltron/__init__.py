...
class StiebelEltronAPI:
    ...
    def __init__(self, register_blocks: list[ModbusRegisterBlock], host: str, port: int = 502, slave: int = 1) -> None:
    ...
    def convert_value(self, register, register_description:ModbusRegister) -> float | int | None:
    ...
    async def async_update(self):
        ...
        for registerblock in self._register_blocks:
            heat_pump_data = await self.read_input_registers(slave=self._slave, address=registerblock.base_address, count=registerblock.count)
            if not heat_pump_data.isError():
                for i in range(0, registerblock.count):
                    descriptor = get_register_descriptor(registerblock.registers.values(), i+registerblock.base_address+1)
                    if descriptor is not None:
                        result[descriptor.key] =  self.convert_value(heat_pump_data.registers[i], descriptor)
        self._data = result        